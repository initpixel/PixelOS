#include <stdint.h>
#include <string.h>

// Экспортируем метку стека из ASM (уже сделано в ASM через global)
extern uint32_t stack_space;

// Структура TSS для возврата из Ring 3 в Ring 0
struct tss_entry_struct {
    uint32_t prev_tss; uint32_t esp0; uint32_t ss0; uint32_t esp1; 
    uint32_t ss1; uint32_t esp2; uint32_t ss2; uint32_t cr3; 
    uint32_t eip; uint32_t eflags; uint32_t eax; uint32_t ecx; 
    uint32_t edx; uint32_t ebx; uint32_t esp; uint32_t ebp; 
    uint32_t esi; uint32_t edi; uint32_t es; uint32_t cs; 
    uint32_t ss; uint32_t ds; uint32_t fs; uint32_t gs; 
    uint32_t ldt; uint16_t trap; uint16_t iomap_base;
} __attribute__((packed));

struct tss_entry_struct tss_entry;

// Инициализация TSS
void init_tss(uint32_t ss0, uint32_t esp0) {
    memset(&tss_entry, 0, sizeof(tss_entry));
    tss_entry.ss0  = ss0;
    tss_entry.esp0 = esp0;
    tss_entry.iomap_base = sizeof(tss_entry);
}

// Инициализация GDT (обязательно для работы jump_to_user_mode)
void init_gdt() {
    // В 2025 году здесь должен быть код заполнения таблицы GDT
}

// Инициализация IDT (обязательно для работы int 0x80)
void init_idt() {
    // Здесь будет регистрация syscall_handler
}

#include <stdint.h>
#include <string.h>

extern uint32_t stack_space;

struct tss_entry_struct {
    uint32_t prev_tss; uint32_t esp0; uint32_t ss0; uint32_t esp1; 
    uint32_t ss1; uint32_t esp2; uint32_t ss2; uint32_t cr3; 
    uint32_t eip; uint32_t eflags; uint32_t eax; uint32_t ecx; 
    uint32_t edx; uint32_t ebx; uint32_t esp; uint32_t ebp; 
    uint32_t esi; uint32_t edi; uint32_t es; uint32_t cs; 
    uint32_t ss; uint32_t ds; uint32_t fs; uint32_t gs; 
    uint32_t ldt; uint16_t trap; uint16_t iomap_base;
} __attribute__((packed));

struct tss_entry_struct tss_entry;